-- Dashboard OPS - Setup Supabase Auth + Inventaire
-- Version SANS DROP: copie-colle tout, ne supprime aucune ligne.
-- Cette version n'efface aucune table, aucune donnee, aucune policy.
-- Elle ajoute les tables, fonctions, restaurants, sources CSV et policies manquantes.

create extension if not exists pgcrypto;

-- =========================================================
-- Helpers: creer une policy/trigger seulement si elle n'existe pas
-- =========================================================

create or replace function public.ops_create_policy_if_missing(
  policy_table text,
  policy_name text,
  policy_sql text
)
...
create or replace function public.ops_create_trigger_if_missing(
  trigger_name text,
  trigger_sql text
)
returns void
...
select public.ops_create_policy_if_missing(
  'profiles',
  'profiles_select_self_or_admin_v89',
  'create policy "profiles_select_self_or_admin_v89" on public.profiles for select to authenticated using (id = auth.uid() or public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'profiles',
  'profiles_insert_self_user_v89',
  'create policy "profiles_insert_self_user_v89" on public.profiles for insert to authenticated with check (id = auth.uid() and role = ''user'')'
);

select public.ops_create_policy_if_missing(
  'profiles',
  'profiles_admin_update_v89',
  'create policy "profiles_admin_update_v89" on public.profiles for update to authenticated using (public.is_super_admin()) with check (public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'restaurants',
  'restaurants_select_authenticated_v89',
  'create policy "restaurants_select_authenticated_v89" on public.restaurants for select to authenticated using (active = true or public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'restaurants',
  'restaurants_admin_write_v89',
  'create policy "restaurants_admin_write_v89" on public.restaurants for all to authenticated using (public.is_super_admin()) with check (public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'user_restaurants',
  'user_restaurants_select_self_or_admin_v89',
  'create policy "user_restaurants_select_self_or_admin_v89" on public.user_restaurants for select to authenticated using (user_id = auth.uid() or public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'user_restaurants',
  'user_restaurants_admin_write_v89',
  'create policy "user_restaurants_admin_write_v89" on public.user_restaurants for all to authenticated using (public.is_super_admin()) with check (public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'user_sheet_sources',
  'sheet_sources_select_self_or_admin_v89',
  'create policy "sheet_sources_select_self_or_admin_v89" on public.user_sheet_sources for select to authenticated using (user_id = auth.uid() or public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'user_sheet_sources',
  'sheet_sources_admin_write_v89',
  'create policy "sheet_sources_admin_write_v89" on public.user_sheet_sources for all to authenticated using (public.is_super_admin()) with check (public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'user_restaurant_requests',
  'restaurant_requests_insert_self_v89',
  'create policy "restaurant_requests_insert_self_v89" on public.user_restaurant_requests for insert to authenticated with check (user_id = auth.uid())'
);

select public.ops_create_policy_if_missing(
  'user_restaurant_requests',
  'restaurant_requests_select_self_or_admin_v89',
  'create policy "restaurant_requests_select_self_or_admin_v89" on public.user_restaurant_requests for select to authenticated using (user_id = auth.uid() or public.is_super_admin())'
);
...
select public.ops_create_trigger_if_missing(
  'products_inventory_value_trigger_v89',
  'create trigger products_inventory_value_trigger_v89 before insert or update on public.products for each row execute function public.products_set_inventory_value()'
);

select public.ops_create_trigger_if_missing(
  'purchase_orders_touch_updated_at_v89',
  'create trigger purchase_orders_touch_updated_at_v89 before update on public.purchase_orders for each row execute function public.touch_updated_at()'
);

select public.ops_create_trigger_if_missing(
  'recipes_touch_updated_at_v89',
  'create trigger recipes_touch_updated_at_v89 before update on public.recipes for each row execute function public.touch_updated_at()'
);
...
select public.ops_create_policy_if_missing(
  'products',
  'products_select_authenticated_v89',
  'create policy "products_select_authenticated_v89" on public.products for select to authenticated using (active_status = true or public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'products',
  'products_admin_write_v89',
  'create policy "products_admin_write_v89" on public.products for all to authenticated using (public.is_super_admin()) with check (public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'inventory_counts',
  'inventory_counts_select_allowed_restaurant_v89',
  'create policy "inventory_counts_select_allowed_restaurant_v89" on public.inventory_counts for select to authenticated using (public.can_access_restaurant(restaurant_id))'
);

select public.ops_create_policy_if_missing(
  'inventory_counts',
  'inventory_counts_insert_allowed_restaurant_v89',
  'create policy "inventory_counts_insert_allowed_restaurant_v89" on public.inventory_counts for insert to authenticated with check (public.can_access_restaurant(restaurant_id) and (counted_by is null or counted_by = auth.uid() or public.is_super_admin()))'
);

select public.ops_create_policy_if_missing(
  'inventory_counts',
  'inventory_counts_update_allowed_restaurant_v94',
  'create policy "inventory_counts_update_allowed_restaurant_v94" on public.inventory_counts for update to authenticated using (public.can_access_restaurant(restaurant_id)) with check (public.can_access_restaurant(restaurant_id))'
);

select public.ops_create_policy_if_missing(
  'purchase_orders',
  'purchase_orders_select_allowed_restaurant_v89',
  'create policy "purchase_orders_select_allowed_restaurant_v89" on public.purchase_orders for select to authenticated using (public.can_access_restaurant(restaurant_id))'
);

select public.ops_create_policy_if_missing(
  'purchase_orders',
  'purchase_orders_insert_allowed_restaurant_v89',
  'create policy "purchase_orders_insert_allowed_restaurant_v89" on public.purchase_orders for insert to authenticated with check (public.can_access_restaurant(restaurant_id) and (created_by is null or created_by = auth.uid() or public.is_super_admin()))'
);

select public.ops_create_policy_if_missing(
  'purchase_orders',
  'purchase_orders_update_allowed_restaurant_v89',
  'create policy "purchase_orders_update_allowed_restaurant_v89" on public.purchase_orders for update to authenticated using (public.can_access_restaurant(restaurant_id)) with check (public.can_access_restaurant(restaurant_id))'
);

select public.ops_create_policy_if_missing(
  'purchase_order_items',
  'purchase_order_items_select_allowed_order_v89',
  'create policy "purchase_order_items_select_allowed_order_v89" on public.purchase_order_items for select to authenticated using (exists (select 1 from public.purchase_orders po where po.id = purchase_order_id and public.can_access_restaurant(po.restaurant_id)))'
);

select public.ops_create_policy_if_missing(
  'purchase_order_items',
  'purchase_order_items_write_allowed_order_v89',
  'create policy "purchase_order_items_write_allowed_order_v89" on public.purchase_order_items for all to authenticated using (exists (select 1 from public.purchase_orders po where po.id = purchase_order_id and public.can_access_restaurant(po.restaurant_id))) with check (exists (select 1 from public.purchase_orders po where po.id = purchase_order_id and public.can_access_restaurant(po.restaurant_id)))'
);

select public.ops_create_policy_if_missing(
  'recipes',
  'recipes_select_authenticated_v89',
  'create policy "recipes_select_authenticated_v89" on public.recipes for select to authenticated using (active_status = true or public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'recipes',
  'recipes_admin_write_v89',
  'create policy "recipes_admin_write_v89" on public.recipes for all to authenticated using (public.is_super_admin()) with check (public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'recipe_ingredients',
  'recipe_ingredients_select_authenticated_v89',
  'create policy "recipe_ingredients_select_authenticated_v89" on public.recipe_ingredients for select to authenticated using (true)'
);

select public.ops_create_policy_if_missing(
  'recipe_ingredients',
  'recipe_ingredients_admin_write_v89',
  'create policy "recipe_ingredients_admin_write_v89" on public.recipe_ingredients for all to authenticated using (public.is_super_admin()) with check (public.is_super_admin())'
);

select public.ops_create_policy_if_missing(
  'inventory_import_logs',
  'inventory_import_logs_select_admin_v89',
  'create policy "inventory_import_logs_select_admin_v89" on public.inventory_import_logs for select to authenticated using (public.is_super_admin() or imported_by = auth.uid())'
);

select public.ops_create_policy_if_missing(
  'inventory_import_logs',
  'inventory_import_logs_insert_self_v89',
  'create policy "inventory_import_logs_insert_self_v89" on public.inventory_import_logs for insert to authenticated with check (imported_by is null or imported_by = auth.uid() or public.is_super_admin())'
);

-- =========================================================
-- Verification rapide: cette requete doit retourner les tables principales.
-- =========================================================

select
  'setup_ok' as status,
  (select count(*) from public.restaurants) as restaurants,
  (select count(*) from public.profiles where lower(email) = lower('a.cloutier@salvatore.com')) as admin_profiles;
